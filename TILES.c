...
int main(int argc, char* argv[]) {
	...
	/* Go through each tile and create a separate file */
	uint16_t tile_width;
	...
	for (current_tile = 0; current_tile < tile_count; current_tile++) {
		...
		/* Go through the data, matching the palette and write to surface */
		uint8_t src_byte;
		...
		printf("Saving %s as a bitmap (%d x %d)", fout, tile_width, tile_height);
